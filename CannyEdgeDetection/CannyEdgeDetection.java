...
public class CannyEdgeDetection {
    ...
    public CannyEdgeDetection(int _lines, int _samples) {
        lines = _lines;
        samples = _samples;
        inputImage = new int[lines][samples];
        System.out.println(lines + " " + samples);
    ...
    public CannyEdgeDetection(int[][] _image) {
        lines = _image.length;
    ...
    public CannyEdgeDetection(String filename) throws IOException {
    ...
    public void next_points(int i, int j, int[][] x_dv, int[][] y_dv,
        ...
        yb = xss * yss;
        if (Math.abs(xs) > (Math.abs(ys) * 2))
            yb = 0;
        if (Math.abs(ys) > (Math.abs(xs) * 2))
            xb = 0;
        v = Math.abs((xb * 3) + yb);
        if (v != 1) {
        ...
        for (i = 0; i < lines; ++i) {
            for (j = 0; j < samples; ++j) {
                if (x_dv[i][j] == 0) {
                    ...
                    switch (s) {
                        case -1:
                            angles[i][j] = (int) (factor * -(3.1415926 * .5));
                            break;
                        case 0:
                            angles[i][j] = 0;
                            break;
                        case 1:
                            angles[i][j] = (int) (factor * (3.1415926 * 0.5));
                            break;
                    }
                } else {
    ...
    public static void main(String[] args) {
        ...
        CannyEdgeDetection canny[] = { null, null, null };
        try {
            String filename = "C:\\Code\\CSC4ST-ImageProcessing\\CannyEdgeDetection\\Space.png";
            ...
            for (int c = 0; c < RGB.length; ++c) {
                canny[c] = new CannyEdgeDetection(RGB[c]);

                // -- set very low thresholds just to see what happens
                int t0 = 50;
                int t1 = 80;
                canny[c].SetThresholds(t0, t1);

                int[][] edges = canny[c].Apply();

                for (int i = 0; i < lines; ++i) {
                    for (int j = 0; j < samples; ++j) {
                        RGB[c][i][j] = (edges[i][j] == 1) ? 255 : 0;
                    }
                }
            }
            BufferedImage oImg = ImageEditor.int2bi(RGB);
            ImageIO.write(oImg, "PNG", new File("C:\\Code\\CSC4ST-ImageProcessing\\CannyEdgeDetection\\ThresholdThree.png"));
